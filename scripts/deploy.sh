...
# ios test
EXPO_APPLE_TEAM_ID="XJA7HDCMMY" bun run eas build --platform ios --profile test --local --non-interactive --output=./app-test.abd && bun run eas submit -p ios --path ./app-test.abd


# ios production
EXPO_APPLE_TEAM_ID="XJA7HDCMMY" bun run eas build --platform ios --profile production --local --non-interactive --output=./app-production.abd && bun run eas submit -p ios --path ./app-production.abd