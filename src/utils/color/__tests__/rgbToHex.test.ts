...
describe('rgbToHex', () => {
  test('converts valid RGB string to hex', () => {
  ...
  test('handles RGB values with spaces', () => {
    expect(rgbToHex('rgb( 255 , 0 , 0 )')).toBe('#ff0000');
    expect(rgbToHex('rgb(26, 201, 162 )')).toBe('#1ac9a2');
  });

  test('handles invalid RGB values by defaulting to 255', () => {
    expect(rgbToHex('rgb(invalid, 0, 0)')).toBe('#ff0000');
    expect(rgbToHex('rgb(0, 0, invalid)')).toBe('#0000ff');
  });

  test('returns the input string if it is not in RGB format', () => {
  ...
  test('handles RGB values with decimal points', () => {
