...
describe('elapsedStreamTime', () => {
  ...
  test('returns minutes when elapsed time is less than one hour', () => {
    const start = subMinutes(NOW, 45).toISOString();
    expect(elapsedStreamTime(start)).toEqual('45m');
  });

  test('returns hours and minutes when the elapsed time is exactly one hour', () => {
    const start = subMinutes(NOW, 60).toISOString();
    expect(elapsedStreamTime(start)).toEqual('1h 00m');
  });

  test('returns hours and minutes when the elapsed time is more than one hour', () => {
    const start = subMinutes(NOW, 125).toISOString();
    expect(elapsedStreamTime(start)).toBe('2h 05m');
  });

  test('returns hours and minutes when the elapsed time is more than 24 hours', () => {
