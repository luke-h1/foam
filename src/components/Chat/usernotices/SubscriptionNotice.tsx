...
import { buildSubscriptionNoticeDescription } from './util/buildSubscriptionNoticeDescription';
...
function renderMessagePart(messagePart: ParsedPart, occurrence: number) {
  ...
  switch (messagePart.type) {
    case 'text':
      return (
        <Text key={key} style={subscriptionNoticeStyles.messageText}>
          {messagePart.content}
        </Text>
      );
    ...
    case 'emote':
      return (
        <Image
          ...
          cacheVariant='emote'
          style={subscriptionNoticeStyles.emote}
        />
      );
    default:
      return null;
...
function SubscriptionNoticeComponent({
  ...
  return (
    <View style={chatStyles.subscriptionNoticeColumn}>
      ...
      >
        <View style={subscriptionNoticeStyles.descriptionContainer}>
          <Text style={subscriptionNoticeStyles.username}>{displayName}</Text>
          {description}
        </View>
      </ChatNoticeMetaRow>
      {(parsedMessage && parsedMessage.length > 0) || message ? (
        <View style={subscriptionNoticeStyles.messageContainer}>
          {renderedParsedMessageParts}
          {!renderedParsedMessageParts && message ? (
            <Text style={subscriptionNoticeStyles.messageText}>
              {message.trim()}
            </Text>
          ) : null}
        </View>
