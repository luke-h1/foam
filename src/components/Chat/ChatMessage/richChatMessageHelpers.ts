...
export function resolveChatBodyVariant(
  detectedVariant: ChatBodyVariant,
  noticeMsgId?: string,
): ChatBodyVariant {
  if (
    detectedVariant === 'twitch_system_notice' &&
...
export function getChatBodyInfo(
  ...
  for (const part of message) {
    if (
      ...
      part.type === 'mention' &&
      normaliseChatUsername(part.content) === normalisedCurrentUsername
    ) {
