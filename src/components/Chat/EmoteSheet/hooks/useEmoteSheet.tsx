...
import { EmotePickerItem } from '../components/EmoteCell';
...
export function useEmoteSheet({
  ...
  let effectiveActiveProviderId = providers[0]?.id ?? null;

  if (
    activeProviderId &&
    providers.some(provider => provider.id === activeProviderId)
  ) {
    effectiveActiveProviderId = activeProviderId;
  }
  ...
  useEffect(() => {
    ...
    const warmupTimer = setTimeout(() => {
      void cacheEmoteImages(preloadVisibleEmotes, abort.signal, 'background');
