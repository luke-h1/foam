...
import {
  ChatInputSection,
  type ReplyToData,
} from './ChatInputSection/ChatInputSection';
...
import { getChatCommandFeedback, parseChatCommand } from './util/chatCommands';
import { buildTimeoutCommandDraft } from './util/timeoutDuration';
...
interface ChatInputShellProps {
  ...
  sendAction: (channel: string, action: string) => void;
  sendChatCommand: (channel: string, command: string) => void;
  sendMessage: (
    channel: string,
    message: string,
...
export const ChatInputShell = memo(function ChatInputShell({
  ...
  const commandContext = useMemo(
    ...
    [canModerateChat, isBroadcaster],
  );
  const commandFeedback = useMemo(
    () => getChatCommandFeedback(messageInput, commandContext),
    [commandContext, messageInput],
  );
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    if (trimmedInput.startsWith('/')) {
      const parsed = parseChatCommand(trimmedInput, commandContext);
      if (parsed) {
        if (!parsed.ok) {
          return;
        }

        if (parsed.delivery === 'action') {
          sendAction(channelName, parsed.message);
        } else {
          sendChatCommand(channelName, parsed.command);
        }
      ...
      sendChatCommand(channelName, trimmedInput);
      clearDraft();
      void KeyboardController.dismiss();
      return;
    }
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      prepareTimeoutCommand: login => {
        if (!isAuthenticated || !canModerateChat) {
          return;
        }

        const command = buildTimeoutCommandDraft(login);
        setDraft(prev => {
          chatInputRef.current?.setText(command);
          return {
            ...prev,
    ...
    [canModerateChat, isAuthenticated],
  );
