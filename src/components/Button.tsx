...
function useButton<Props extends ButtonProps>(propsIn: Props) {
  // order is important, be careful not to destructure and re-order props
  ...
  const contents = wrapChildrenInText(CustomButtonText, propsActive, {
    size,
  ...
  const tag = isNested
    ? 'span' // defaults to <a /> when accessibilityRole = link
      ...
      <CustomButtonFrame
        ...
        onPressIn={
          buttonProps.hapticFeedback
            ? () => {
                hapticFeedback(buttonProps.hapticStyle);
              }
            : undefined
        }
        {...buttonProps}
