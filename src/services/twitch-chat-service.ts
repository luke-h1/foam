...
export function useTwitchChat(options: UseTwitchChatOptions = {}) {
        ...
        case 'USERNOTICE': {
          ...
          if (params.length >= 2 && tags) {
            ...
            if (channelName) {
              ...
              onUserNotice?.(channelName, tagsRecord, messageText || '');
            }
          }
          break;
        }
  ...
  useEffect(() => {
    ...
    if (wasOnChatScreen && !isOnChatScreen) {
      ...
      // Channels will be parted automatically when WebSocket closes
  ...
  const sendMessage = useCallback(
    (
      ...
      // Store pending message info for USERSTATE callback
