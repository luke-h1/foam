...
## Chat sheets run on `@expo/ui/community/bottom-sheet`

`src/components/BottomSheet/BottomSheet.native.tsx` wraps the `@expo/ui` drop-in bottom sheet (SwiftUI `.sheet` on iOS, Material 3 `ModalBottomSheet` on Android). This is an interim swap away from `@swmansion/react-native-bottom-sheet`, which froze the app when a sheet was dismissed. `@swmansion/react-native-bottom-sheet` and its `bottomInset`/`cornerRadius` native patch stay installed and unimported so the swap is JS-only and revertible over the air - do not delete the dependency or the patch to "clean up an unused package".

The sheet always runs in fit-to-content mode. An explicit content height is the only sizing model a SwiftUI height detent and a Compose `ModalBottomSheet` agree on, so the wrapper resolves the first snap point to a pixel height and puts it on the content `View` instead of passing `snapPoints` through. Passing snap points through would leave Android sizing content against a sheet that is itself sizing to that content, and only the first two of them mean anything there anyway.

Sheet chrome is native now: the drag indicator, the scrim, and the corner radius all come from the platform, and `enablePanDownToClose` is what turns on both swipe-to-dismiss and scrim-tap dismiss (SwiftUI cannot separate the two). Do not reintroduce a JS drag pill, a `Pressable` backdrop, or an RN `Modal` wrapper. The Android-only in-sheet `Toaster` is still needed - `ModalBottomSheet` presents in its own window, so toasts fired from a sheet (badge/emote copy and save) render behind the root `Toaster` without it.

`onDismiss` fires when the dismissal starts, not when it finishes. Every consumer is unmounted by `ChatOverlayLayer` on `onDismiss`, so the sheet unmounts during its own close animation. That is fine because a dismissal is always already in flight by then; unmounting a sheet that is still fully presented is what strands a native sheet on screen.
